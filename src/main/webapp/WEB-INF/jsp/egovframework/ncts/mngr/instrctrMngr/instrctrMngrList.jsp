...
<script type="text/javascript">
	$(function(){
	    ...
	    $.dataDetail = function(index){
	        ...
	        $.ajax({
	            type: 'POST',
	            url: "/ncts/mngr/instrctrMngr/mngrInstrctrDetail.do",
	            ...
	            success: function(data) {
	                if(data.success == "success"){
	                    ...
	                    $(".instrctrInput").instrctrInputOnChangeEvt(baseInfo.updateKey, index);
	    ...
	    $.fn.changeDetailBtnOnClickEvt = function(){
	    	$(this).on("click", "#changeDetailBtn", function(){
	    		var $this = $(this);
	    ...
	    $.excelDownload = function(e){
	    	excelPg = 1;
			$("[name='excelFileNm']").val("강사(명단)관리_"+$.toDay());
	    ...
	    $.pfatInstrctrEntrstDeOnCloseSetting = function(){
			
			var $pfatInstrctrEntrstDe = $("input[data-name='pfatInstrctrEntrstDe']");
			$pfatInstrctrEntrstDe.datepicker("option", "onClose", function (selectedDate){
				var today = moment(new Date());
				var todayDate = today.format("YYYY.MM.DD");
					
				if(selectedDate != "" && selectedDate > todayDate) {
					alert("정확한 날짜를 입력해주세요.");
					$pfatInstrctrEntrstDe.val("");
					$pfatInstrctrEntrstDe.next("input").val("");
				}
			});
		}
	    ...
	    $.initView = function(){
	        ...
	        $("body").changeDetailBtnOnClickEvt();
	...
	<section id="widget-grid" class="">
		...
		<div class="content">
			...
			<div class="row">
				...
				<article class="col-md-12 col-lg-6">
					<table class="table table-bordered tb_type03" style="table-layout: fixed;">
						<colgroup>
							<col width="10%">
							<col width="20%">
							<col width="13%">
...
<tr>
	...
	<th scope="row">PFA</th>
	<td>
		<label class="select col w150" style="display:inline;">
	...
	{{#ifeq PFAT_GRADE_CD '01'}}
  	  <label class="select col w100" style="display:inline;">
	...
	{{#ifeq PFAT_GRADE_CD '00'}}
   		<label class="input col w70 fRight" style="line-height: 12px;font-size: 12px;">위촉일자

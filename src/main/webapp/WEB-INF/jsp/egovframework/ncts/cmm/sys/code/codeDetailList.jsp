...
$(function(){
	...
	$.dataDetail = function(index){
		...
		document.sForm.code.value = index;
	}
		...
		_this.on("click", function(){
			if(baseInfo.insertKey != key){
				if($("input.index:checked").size() <= 0) {
					...
					document.sForm.code.value = $("input.index:checked").val();
					document.sForm.codeId.value = $("input.index:checked").next().val();
				} 
			}else{
				document.sForm.code.value = "";
			}
	...
	$.fn.codeIdTdOnClickEvt = function(){
		$(this).on("click", function(){
			var codeIdVal = $(this).closest("tr").find(".index").next("input[type='hidden']").val();
			with(document.sForm){
	            searchKeyword3.value = codeIdVal;
	            action = "/ncts/cmm/sys/code/codeList.do";
				target='';
	            submit();
	        }
		})
	...
	<section id="widget-grid" class="">
		...
		<div class="search">
          	<form name="sForm" id="sForm" method="post">
				...
				<input type="hidden" name="searchKeyword3" id="searchKeyword3"  value="">
				<div class="fL wp50">
